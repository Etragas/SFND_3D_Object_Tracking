    ...
    for (auto detectorType : detectors)
    {
        for (auto descriptorType : descriptors)
        {
            try
            {
                ...
                std::vector<double> ttc_lidar_accumul;
                std::vector<double> ttc_cam_accumul;
                int steps = 0;
                vector<int> kp_sizes;
                vector<DataFrame> dataBuffer; // list of data frames which are held in memory at the same time
                    ...
                    if (dataBuffer.size() > 1) // wait until at least two images have been processed
                    {
                        ...
                        for (auto it1 = (dataBuffer.end() - 1)->bbMatches.begin(); it1 != (dataBuffer.end() - 1)->bbMatches.end(); ++it1)
                        {
                            ...
                            if (currBB->lidarPoints.size() > 0 && prevBB->lidarPoints.size() > 0) // only compute TTC if we have Lidar points
                            {
                                ...
                                ttc_lidar_accumul.push_back(ttcLidar);
                                //// EOF STUDENT ASSIGNMENT
                                ...
                                ttc_cam_accumul.push_back(ttcCamera);
                                //// EOF STUDENT ASSIGNMENT

                                bVis = true;
                                if (bVis)
                                {
                                    show3DObjects((dataBuffer.end() - 1)->boundingBoxes, cv::Size(4.0, 20.0), cv::Size(2000, 2000), "_curr" , false);
                                    show3DObjects((dataBuffer.end() - 2)->boundingBoxes, cv::Size(4.0, 20.0), cv::Size(2000, 2000), "_prev" , false);
                                    ...
                                    cout << "Current step is " << to_string(steps) << endl;
